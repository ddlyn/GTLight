...
class ConstructSample:
    ...
    def load_data_for_system(self, folder):
        ...
        self.measure_time = self.dic_traffic_env_conf["MEASURE_TIME"]#10
        self.interval = self.dic_traffic_env_conf["MIN_ACTION_TIME"]#10

        for i in range(self.dic_traffic_env_conf['NUM_INTERSECTIONS']):#36
            pass_code, logging_data = self.load_data(folder, i)#导入所有仿真获得的数据，s，a
            if pass_code == 0:
                return 0
