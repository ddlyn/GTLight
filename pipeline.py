...
class Pipeline:
    ...
    def run(self, multi_process=False):
        ...
        if self.dic_exp_conf["PRETRAIN"]:#预训练
            if os.listdir(self.dic_path["PATH_TO_PRETRAIN_MODEL"]): 
                ...
                if not os.listdir(self.dic_path["PATH_TO_PRETRAIN_WORK_DIRECTORY"]):
                    for cnt_round in range(self.dic_exp_conf["PRETRAIN_NUM_ROUNDS"]):#预训练回合数
        ...
        # trainf
        for cnt_round in range(self.dic_exp_conf["NUM_ROUNDS"]):#100
            print("round %d starts" % cnt_round)
            ...
            if multi_process:
                ...
                for cnt_gen in range(self.dic_exp_conf["NUM_GENERATORS"]):#4,?
                    self.generator_wrapper(cnt_round=cnt_round,
                                           cnt_gen=cnt_gen,
                                           dic_path=self.dic_path,
                                           dic_exp_conf=self.dic_exp_conf,
