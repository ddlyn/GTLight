...
from keras.layers import Dense, Dropout, Conv2D, Input, Lambda, Flatten, TimeDistributed, merge,normalization
...
class transformerAgent(Agent): 
    ...
    def MultiHeadsAttModel(self,In_agent,In_neighbor,l=5, d=128, dv=16, dout=128, nv = 8,suffix=-1):
        ...
        agent_repr=Reshape((self.num_agents,1,d))(In_agent)#(;36;1;32)
        ...
        Residual=Lambda(lambda x:K.sum(x,axis=2))(agent_repr_head)
        ...
        out=Lambda(lambda x:K.batch_dot(x[0],x[1],axes=[4,3]))([att,neighbor_hidden_repr_head])
        out=Lambda(lambda x:K.mean(x,axis=2))(out)
        FF_in= Add()([out,Residual])
        FF_in=Reshape((self.num_agents,dv))(FF_in)
        FF_in=normalization.BatchNormalization()(FF_in)
        FF_h=Dense(dv,activation='relu',kernel_initializer='random_normal')(FF_in)
        FF_out=Dense(dv,activation=None,kernel_initializer='random_normal')(FF_h)
        out=Add()([FF_out,FF_in])
        out=normalization.BatchNormalization()(out)
    ...
    def action_att_predict(self,state,total_features=[],total_adjs=[],bar=False):
        ...
        #out: [batch,agent,action], att:[batch,layers,agent,head,neighbors]#(40, 4, 36, 1, 5)
        if len(action)>1:
        ...
        for i in range(len(sample_slice)):
            for j in range(self.num_agents):
                q_values[i][j][_action[i][j]] = _reward[i][j] / self.dic_agent_conf["NORMAL_FACTOR"] + self.dic_agent_conf["GAMMA"] * \
                                     target_q_values[i][j][np.argmax(q_values[i][j])]
    ...
    def build_network(
        ...
        CNN_heads=[4]*len(CNN_layers)#toushu
        """
        layer definition
        """
